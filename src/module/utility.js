...
export class DEG_Utility {
  static addDiamonds(data, diamondMax = 0, valueAttribute = "value") {
    ...
    for (let i = 0; i < diamondMax; i++) {
      data.diamonds.push({
        filled: i + 1 <= foundry.utils.getProperty(data, valueAttribute),
      });
    }

    if (foundry.utils.getProperty(data, "max")) {
      for (let i = 0; i < diamondMax; i++) {
        data.diamonds[i].locked =
          i + 1 > foundry.utils.getProperty(data, "max");
      }
    }
  ...
  static getModificationActions() {
    ...
    for (let skill in DEGENESIS.skills)
      actions[`skill:${skill}`] = `${
        DEGENESIS.attributeAbbrev[DEGENESIS.skillAttributes[skill]]
